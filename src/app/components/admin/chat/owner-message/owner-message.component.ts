...
@Component({
  ...
  styleUrl: './owner-message.component.css'
})
export class OwnerMessageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log("This is owner Id - ", this.userId);
    this.getOwnerInfo(this.userId);
    this.loadMessages();
    this.setupWebSocket();
  }
  ...
  getOwnerInfo(userId : number): void{
    this.userService.getUserById(userId).subscribe({
      next: (owner) => {
        this.ownerDetails = owner;
        this.loadMessages();
      },
      error: (err) => console.error('Error fetching owner info:', err),
    });
  }
  ...
  loadMessages(): void {
      ...
      this.messageService.getChatMessages(this.loggedInUserId, this.userId).subscribe({
        next: (messages) => {
